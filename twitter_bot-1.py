...
# ── Owner(s) — same pattern as the main WIZRAD Scan bot ─────────────────
...
DEFAULT_START_TEXT = (
    ...
    "Yahan se aap X (Twitter) ke top calls aur trending posts follow kar sakte "
    "hain — live retweet/follower milestones aur token-call X-alerts ke saath.\n\n"
    "📤 Apni koi tweet submit karni ho? Bas wo link seedha yahan (DM mein) bhej "
    "dein — hamari team review kar ke channel par post karegi.\n\n"
    "❓ Saari commands ke liye /help bhejein."
)
...
PUBLIC_HELP_TEXT = (
    ...
    "/start — bot ka intro\n"
    "/help — yehi list\n"
    "/mystatus — apni last submitted request ka status\n\n"
    "📤 Kisi bhi X/Twitter post ka link seedha yahan (DM mein) bhej dein — "
    "hum use review kar ke accept/reject karenge."
)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Tweet Submit Karein", callback_data="pub:howsubmit")],
...
async def cmd_mystatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mine:
        await update.message.reply_text("Aap ne abhi tak koi tweet submit nahi ki.")
        return
    last = mine[-1]
    status_map = {"pending": "⏳ Pending review", "accepted": "✅ Accepted & posted",
                  "rejected": "❌ Rejected"}
    await update.message.reply_text(f"Aapki last request: {status_map.get(last['status'], last['status'])}")
...
async def cb_public(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "pub:help":
        ...
        await q.message.reply_text(
            "📤 Bas tweet ya x.com ka link yahan (isi DM mein) bhej dein.\n"
            "Hamari team dekh kar accept/reject karegi — /mystatus se status "
            "check kar sakte hain.", parse_mode=ParseMode.HTML)
...
async def handle_private_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tweet_id in tracked_tweets:
        await update.message.reply_text("⏭️ Ye tweet pehle hi channel par post ho chuki hai.")
        return
    for r in pending_reqs.values():
        if r.get("tweet_id") == tweet_id and r.get("status") == "pending":
            await update.message.reply_text("⏳ Ye request already review mein hai, thora intezar karein.")
            return
    ...
    await update.message.reply_text("✅ Aapki request bhej di gayi hai — owner review karega. Shukriya!")
...
async def cb_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not req or req.get("status") != "pending":
        await q.answer("Ye request already handle ho chuki hai.", show_alert=True)
        return

    if action == "acc":
        tweet = await fetch_tweet(req["tweet_id"])
        if not tweet:
            await q.answer("⚠️ Tweet fetch nahi ho saka (deleted/private).", show_alert=True)
            return
        try:
            await _post_accepted_tweet(tweet, context)
        except Exception as e:
            await q.answer(f"❌ Channel post fail: {e}", show_alert=True)
            return
        req["status"] = "accepted"
        _save_pending()
        ...
        try:
            await context.bot.send_message(
                req["user_id"], f"✅ Aapki tweet post ho gayi hai channel par!\n{tweet['url']}")
        except Exception:
            pass
    else:
        req["status"] = "rejected"
        ...
        await q.edit_message_text(
            ...
            parse_mode=ParseMode.HTML)
        try:
            await context.bot.send_message(req["user_id"], "❌ Maazrat, aapki request accept nahi ki gayi.")
        except Exception:
            pass
    await q.answer()
...
OWNER_HELP_TEXT = (
    ...
    "📥 <b>Requests</b>\n"
    "/pending — sab open requests dubara dekhein (buttons ke saath)\n\n"
    "📡 <b>X / KOL List</b>\n"
    "/addx handle · /removex handle · /listx\n\n"
    "🎛 <b>Start Menu</b>\n"
    "/setstart — /start ka text + media set karein (wizard)\n\n"
    "🧩 <b>Channel Signal Card</b>\n"
    "/setcard &lt;template&gt; — HTML template\n"
    "Placeholders: <code>{screen_name} {verified_mark} {quote} {url}</code>\n"
    "/showcard · /resetcard\n\n"
    "➕ <b>Apni Custom Commands</b>\n"
    "/newcommand — naya /command banayein (apna text + media + buttons, HTML allowed)\n"
    "/listcommands · /removecommand naam\n\n"
    "🚫 /cancel — kisi bhi wizard ko rok dein"
)
...
async def cb_ownerhelp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = q.data.split(":")[1]
    tips = {
        "pending": "👉 /pending bhejein saari open requests dekhne ke liye.",
        "kols": "👉 /addx handle · /removex handle · /listx",
        "start": "👉 /setstart bhejein — text likhein, phir media ya /skip.",
        "card": "👉 /setcard &lt;template&gt; bhejein, /showcard, /resetcard.",
        "cmds": "👉 /newcommand se naya command banayein, /listcommands · /removecommand naam.",
    }
...
async def cmd_addx(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: /addx handle (bina @ ke)")
        return
    h = context.args[0].lstrip("@").lower()
    if h not in [k.lower() for k in tracked_kols]:
        tracked_kols.append(h)
        save_json(KOLS_FILE, tracked_kols)
    await update.message.reply_text(f"✅ @{h} KOL list mein add ho gaya.")

@owner_only
async def cmd_removex(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global tracked_kols
    if not context.args:
        await update.message.reply_text("Usage: /removex handle")
        return
    h = context.args[0].lstrip("@").lower()
    ...
    if len(tracked_kols) < before:
        await update.message.reply_text(f"🗑 @{h} list se hata diya.")
    else:
        await update.message.reply_text("Ye handle list mein tha hi nahi.")

@owner_only
async def cmd_listx(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not tracked_kols:
        await update.message.reply_text("Koi KOL list mein nahi hai abhi.")
        return
    await update.message.reply_text(
        "📡 <b>KOL List</b>\n" + "\n".join(f"• @{k}" for k in tracked_kols),
        parse_mode=ParseMode.HTML)
...
async def cmd_setstart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✏️ /start ka naya text bhejein (HTML allowed — <b>bold</b>, <i>italic</i>, "
        "<code>&lt;a href='...'&gt;link&lt;/a&gt;</code>).\n🚫 /cancel se rok sakte hain.",
        parse_mode=ParseMode.HTML)
...
async def cmd_setcard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Signal card template update ho gaya.")

@owner_only
async def cmd_showcard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = card_template.get("text")
    if not t:
        await update.message.reply_text("Abhi default template use ho raha hai (koi custom set nahi).")
        return
...
async def cmd_resetcard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_json(CARD_TEMPLATE_FILE, card_template)
    await update.message.reply_text("♻️ Default template par wapas.")
...
async def cmd_newcommand(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "➕ <b>Naya Command</b>\n\nCommand ka naam bhejein (bina / ke, sirf letters/numbers), "
        "misaal: <code>rules</code>\n\n🚫 /cancel se rok sakte hain.", parse_mode=ParseMode.HTML)

@owner_only
async def cmd_listcommands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not custom_cmds:
        await update.message.reply_text("Koi custom command nahi bana abhi tak.")
        return
...
async def cmd_removecommand(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: /removecommand naam")
        return
    ...
    if name in custom_cmds:
        ...
        await update.message.reply_text("Ye command exist nahi karta.")

@owner_only
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    owner_state.pop(update.effective_user.id, None)
    await update.message.reply_text("🚫 Rok diya.")
...
async def _route_owner_state(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "start_text":
        ...
        await update.message.reply_text("🖼 Ab media (photo/video/gif) bhejein, ya /skip likh dein.")
        return
    ...
    if state == "start_media":
        if text == "/skip":
            ...
            if not file_id:
                await update.message.reply_text("Ye media samajh nahi aayi — dobara bhejein ya /skip.")
                return
            start_cfg["text"] = data["text"]
        ...
        owner_state.pop(uid, None)
        await update.message.reply_text("✅ /start update ho gaya.")
        return

    if state == "newcmd_name":
        name = text.lstrip("/").lower()
        if not name.isalnum():
            await update.message.reply_text("Sirf letters/numbers use karein, koi space/symbol nahi. Dobara bhejein:")
            return
        if name in RESERVED_CMDS:
            await update.message.reply_text("Ye naam reserved hai, koi aur naam try karein:")
            return
        data["name"] = name
        st["state"] = "newcmd_text"
        await update.message.reply_text("✏️ Ab is command ka text bhejein (HTML allowed):")
        return

    if state == "newcmd_text":
        data["text"] = update.message.text_html or text
        st["state"] = "newcmd_media"
        await update.message.reply_text("🖼 Media bhejein (photo/video/gif), ya /skip:")
        return
    ...
    if state == "newcmd_media":
        if text == "/skip":
            ...
            file_id, mtype = _extract_media(update.message)
            if not file_id:
                await update.message.reply_text("Media samajh nahi aayi — dobara bhejein ya /skip.")
                return
            data["media_file_id"] = file_id
            data["media_type"] = mtype
        st["state"] = "newcmd_buttons"
        await update.message.reply_text(
            "🔘 Buttons chahiye? Ek line mein ek button:\n<code>Label - https://link.com</code>\n"
            "Ya /skip agar koi button nahi chahiye.", parse_mode=ParseMode.HTML)
        return
    ...
    if state == "newcmd_buttons":
        ...
        save_json(CUSTOM_CMDS_FILE, custom_cmds)
        owner_state.pop(uid, None)
        await update.message.reply_text(f"✅ /{data['name']} ban gaya! Ab sab users use kar sakte hain.")
        return
...
async def _post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("mystatus", "Apni request ka status"),
    ])
