...
async def cmd_addx(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: /addx handle (without @)")
        return
    ...
    await update.message.reply_text(
        f"✅ @{h} added to the KOL list. Auto-watching for new posts every "
        f"~{KOL_POLL_SECONDS // 60} min (best-effort — see /ownerhelp)."
    )
